...
def upload_restaurant_profile():
    ...
    try:
        db.engine.execute(
            "UPDATE customer SET restaurant_name = '%s', restaurant_address = '%s', delivery_price = '%s', base_deliver_price = '%s', open_time = '%s', restaurant_description = '%s' WHERE restaurant_id = '%s'" % \
